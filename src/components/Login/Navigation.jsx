...
function Navigation({ loginTab }) {
    const dispatch = useDispatch();
    return (
        <>
            <button className="sign-wrap__btn" onClick={() => dispatch(setLoginPopup(false))}>Закрити</button>
            <ul className="sign-wrap__list">
                <li className="sign-wrap__item">
                    <button className={`sign-wrap__login-btn  ${loginTab ? 'sign-wrap__active-btn' : ''}`} onClick={() => dispatch(setLoginForm(true))}>
                        <svg className="login-btn--icon" width="20" height="20" viewBox="0 0  20 20" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <g>
                                <path d="M3.75 0V3.75001H6.24998V2.49998H16.25V17.5H6.24998V16.25H3.75V20H18.75V0H3.75Z" fill="#77CCF9">
                                </path>
                                <path
                        ...
                        <span className="sign-wrap__btn-text login-btn">Вхід</span>
                    </button>
                </li>
                <li className="sign-wrap__item">
                    <button className={`sign-wrap__reg-btn ${!loginTab ? 'sign-wrap__active-btn' : ''}`} onClick={() => dispatch(setLoginForm(false))}>
                        ...
                        <span className="sign-wrap__btn-text registration-btn">Реєстрація</span>
                    </button>
                </li>
            </ul>
        </>
